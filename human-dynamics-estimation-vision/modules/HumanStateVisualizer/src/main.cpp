...
int main(int argc, char* argv[])
{
    ...
    std::string visionTargetsWrapperPortName;
    if (visualizeTargets)
    {
        if( !(rf.check("visionTargetsWrapperPortName") && rf.find("visionTargetsWrapperPortName").isString()) )
        {
            yError() << LogPrefix << "'visionTargetsWrapperPortName' option not found or not valid. Wrench Visualization will be disabled";
            visualizeTargets = false;
        }
        else
        {
            visionTargetsWrapperPortName = rf.find("visionTargetsWrapperPortName").asString();
        }
    }
    ...
    // initialize iVisionTargets interface from remapper
    std::unordered_map<hde::TargetName, std::shared_ptr<hde::VisionTarget>> targets;
    yarp::dev::PolyDriver visionTargetsRemapperDevice;
    if (visualizeTargets) {
        hde::interfaces::IVisionTargets* iVisionTargets{nullptr};

        yarp::os::Property visionTargetsRemapperOptions;
        visionTargetsRemapperOptions.put("device", "vision_targets_remapper");
        visionTargetsRemapperOptions.put("visionTargetsDataPort", visionTargetsWrapperPortName);
        std::cerr << "my wrapper port name is: "  << visionTargetsWrapperPortName << std::endl;

        if(!visionTargetsRemapperDevice.open(visionTargetsRemapperOptions))
        {
            yError() << LogPrefix << "Failed to connect remapper device";
            return EXIT_FAILURE;
        }
        if(!visionTargetsRemapperDevice.view(iVisionTargets) || !iVisionTargets )
        {
            yError() << LogPrefix << "Failed to view iVisionTargets interface";
            return EXIT_FAILURE;
        }

        // wait for the iVisionTargets to be initialized
        std::this_thread::sleep_for(std::chrono::seconds(1));
        while (iVisionTargets->getAllTargetsName().empty())
        {
            std::this_thread::sleep_for(std::chrono::seconds(1));
            yInfo() << LogPrefix << "Waiting for data from VisionTargetsRemapper";
        }
        ...
        auto targetsName = iVisionTargets->getAllTargetsName();
        for (auto visualizedTargetFrame : visualizedTargetsFrame)
        {
            auto target = iVisionTargets->getTarget(visualizedTargetFrame);

            // check if target exists
            if (!target)
            {
                yError() << LogPrefix << "target [ " << visualizedTargetFrame << " ] not found in iVisionTargets." ;
                return EXIT_FAILURE;
            }
    ...
    visionTargetsRemapperDevice.close();
    wrenchPort.close();
